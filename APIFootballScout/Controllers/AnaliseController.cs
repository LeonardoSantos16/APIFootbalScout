...
namespace APIFootballScout.Controllers
{
    ...
    public class AnaliseController(
        ConsultarMetricasPor90UseCase consultarMetricasPor90UseCase,
        ComparacaoDiretaUseCase comparacaoDiretaUseCase) : ControllerBase
    {
        ...
        [ProducesResponseType<ProblemDetails>(StatusCodes.Status500InternalServerError)]
        [ProducesResponseType<ProblemDetails>(StatusCodes.Status502BadGateway)]
        public async Task<ActionResult<ComparacaoDiretaResponseDto>> Comparar(
            [FromQuery] ComparacaoDiretaRequestDto dto,
            CancellationToken cancellationToken)
        {
            var result = await comparacaoDiretaUseCase.Comparar(dto.ParaRequest(), cancellationToken);
